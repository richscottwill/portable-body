# Wiki Maintenance Workflow — Protocol
...
1. **Pause flag.** If `<repo-root>/context/active/wiki-maintenance-paused.flag` exists, exit with a visible paused message and do not start connector work.
2. **Concurrency lock.** If `<repo-root>/context/active/wiki-maintenance.lock` is fresh, exit as already running. If stale, overwrite and continue.
...
| # | Phase | Gate |
|---|---|---|
| 1 | Preflight and lock | Stop or degrade visibly if this phase cannot prove its output. |
| 2 | Broad sweep | Stop or degrade visibly if this phase cannot prove its output. |
| 3 | Researcher pass | Stop or degrade visibly if this phase cannot prove its output. |
| 4 | Writer pass | Stop or degrade visibly if this phase cannot prove its output. |
| 5 | Critic/eval pass | Stop or degrade visibly if this phase cannot prove its output. |
| 6 | Revision pass | Stop or degrade visibly if this phase cannot prove its output. |
| 7 | Librarian/publication pass | Stop or degrade visibly if this phase cannot prove its output. |
| 8 | Concierge demand report | Stop or degrade visibly if this phase cannot prove its output. |
| 9 | Maintenance digest | Stop or degrade visibly if this phase cannot prove its output. |
