# Karpathy Compression / Curriculum Workflow — Protocol

Workflow id: `karpathy-compression`
...
1. **Pause flag.** If `<repo-root>/context/active/karpathy-compression-paused.flag` exists, exit with a visible paused message and do not start connector work.
2. **Concurrency lock.** If `<repo-root>/context/active/karpathy-compression.lock` is fresh, exit as already running. If stale, overwrite and continue.
...
| # | Phase | Gate |
|---|---|---|
| 1 | Candidate selection | Stop or degrade visibly if this phase cannot prove its output. |
| 2 | Experiment prompt | Stop or degrade visibly if this phase cannot prove its output. |
| 3 | Structural validity check | Stop or degrade visibly if this phase cannot prove its output. |
| 4 | Blind eval route | Stop or degrade visibly if this phase cannot prove its output. |
| 5 | Score comparison | Stop or degrade visibly if this phase cannot prove its output. |
| 6 | Keep/revert | Stop or degrade visibly if this phase cannot prove its output. |
| 7 | Learning log and cooldown | Stop or degrade visibly if this phase cannot prove its output. |
